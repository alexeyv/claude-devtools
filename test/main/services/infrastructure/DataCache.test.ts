...
describe('DataCache session detail schema version', () => {
  it('invalidates version 3 session details and stamps new entries with version 4', () => {
    ...
    const currentDetail = { swimlane: { schemaVersion: 2 } } as unknown as SessionDetail;
    cache.set(key, currentDetail);

    expect(entries(cache).get(key)?.version).toBe(4);
